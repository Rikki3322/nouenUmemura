...
export const recommendSite = (answers: Answers) => {
  const rakuten = sites.find((site) => site.name.includes('rakuten.name'));
  const tabechoku = sites.find((site) => site.name === 'tabechoku.name');
  const base = sites.find((site) => site.name === 'base.name');
  const pokemaru = sites.find((site) => site.name === 'pokemaru.name');

  if (answers.freeShipping === 'yes' && rakuten) return rakuten;
  if (answers.subscription === 'yes' && tabechoku) return tabechoku;
  if (answers.flexibleAmount === 'yes' && pokemaru) return pokemaru;
